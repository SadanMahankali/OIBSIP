...
2. Visualises it (pairplot + box plots) to see which features separate the species.
3. Split the data 80/20 into train and test sets (stratified, since it's sorted by species).
4. Build and train two classifiers, K-Nearest Neighbours and a Decision Tree.
...
- **Petal length was the most useful feature** for separating species; sepal width the least.
- **Setosa** was completely separated from the other two, while **versicolor** and **virginica** overlapped slightly, which is exactly where the Decision Tree made its only 2 mistakes.
